...
export function Table({ d = [], get = {} }) {
  ...
  const [isErrorOpen, setIsErrorOpen] = useState(false);  // New state for error modal
  ...
  const toggleErrorModal = () => {
    setIsErrorOpen(!isErrorOpen);  // Toggle error modal state
  };
  ...
  const handleSaveClick = async () => {
    if (!DEPARTMENTS.includes(editData.rol)) {  // Use 'rol' instead of 'department'
      setError("Rol no válido. Selecciona un rol de la lista permitida:");
      toggleErrorModal();
      return;
    }
  ...
  return (
    <>
      ...
      <WmModal classN="wm-modal-bg-white wm-modal-center wrap" Open={isErrorOpen} closeButton={{ open: true, toggleModal: toggleErrorModal }}>
        <p>{error}</p>
        <p>Admin</p>
        <p>Director</p>
        <p>Doctor</p>
        <p>Nurse</p>
        <p>Recepcionist</p>
        <button className="wm-button-ok" onClick={toggleErrorModal} style={{ borderRadius: "4px", color: "white", border: "none", padding: "4px 8px", cursor: "pointer", backgroundColor:"#1977cc77", hover: { backgroundColor: "#fcef2d77" } }}>
          OK
        </button>
      </WmModal>
